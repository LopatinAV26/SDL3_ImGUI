...
void Eresco32::Plot(const float &focusDistance, const float &mA, const int &measure_index, const float &exposureMultiplier)
{
...
float Eresco32::CalculateIntensity(const float &intensity_0, const float &focusDistance_0, const float &focusDistance_1)
{
    return intensity_0 / std::pow((focusDistance_0 / focusDistance_1), 2);
}

float Eresco32::ToMinute(const float &exposition, const float &mA)
{
    return exposition / mA;
}

float Eresco32::ToSecond(const float &exposition, const float &mA)
{
